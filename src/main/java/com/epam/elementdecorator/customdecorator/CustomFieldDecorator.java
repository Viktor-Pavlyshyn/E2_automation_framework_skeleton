...
public class CustomFieldDecorator extends DefaultFieldDecorator {
    ...
    protected CustomElement createElement(ClassLoader loader, ElementLocator locator, Class<CustomElement> clazz) {
        WebElement proxy = proxyForLocator(loader, locator);
        return WrapperFactory.createInstance(clazz, proxy);
    }

    @SuppressWarnings("unchecked")
    protected List<CustomElement> createList(ClassLoader loader, ElementLocator locator, Class<CustomElement> clazz) {

        InvocationHandler handler = new LocatingCustomElementListHandler(locator, clazz);
        List<CustomElement> elements = (List<CustomElement>) Proxy.newProxyInstance(
